...
dlg_title = 'Settings';
num_lines = 1;
defaultans = {'1','1','12','2','0'}; % change these values to alter default input!!!
...
for norm = [0 1]
    ...
    if norm == 1  % Apply the normalization
        mean_bs = mean(bs,2);
        bs_p = bs./mean_bs;
        ss_p = ss./mean_bs; 
        ps_p = ps./mean_bs; 
    else % No baseline normalization
        bs_p = bs;
        ss_p = ss; 
        ps_p = ps; 
    end
...
for norm = 0
    ...
    if norm == 1  % Apply the normalization
        save([ouputfolder 'normalized_final_RR.mat'],'RR_group')
    else % No baseline normalization
        save([ouputfolder 'raw_final_RR.mat'],'RR_group')
    end
end 
